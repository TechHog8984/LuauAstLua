...
local isInstance, class, simpleclass, enum, newBuffer = shared.isInstance, shared.class, shared.simpleclass, shared.enum, shared.newBuffer;
local Tokenizer = require("Tokenizer");

local FInt = shared.FInt;
local FFlag = shared.FFlag;

FInt:set("LuauRecursionLimit", 1000);
FInt:set("LuauParseErrorLimit", 100);
FFlag:set("LuauParseDeclareClassIndexer", false);

local getIdentifier = shared.getIdentifier;

local ERROR_INVALID_INTERP_DOUBLE_BRACE = "Double braces are not permitted within interpolated strings. Did you mean '\\{'?";

local AstNameTable = shared.AstNameTable;
local Vector = shared.Vector;
local TokenType = shared.TokenType;
local TempVector = shared.TempVector;
...
local function isExprLValue(expression)
    return  isInstance(expression, AstExpressionLocal) or isInstance(expression, AstExpressionGlobal) or isInstance(expression, AstExpressionIndexExpression) or
            isInstance(expression, AstExpressionIndexName);
end;

local function shouldParseTypePack(tokenizer)
    if tokenizer:getCurrent().token_type == TokenType.Dot3 then
        return true;
    elseif tokenizer:getCurrent().token_type == TokenType.Name and tokenizer:lookAhead().token_type == TokenType.Dot3 then
        return true;
    end;
...
local Parser = class(function(self, content, names, options)
    ...
    parse = function(self)
        local success, result = xpcall(function()
            ...
            local lines = self.tokenizer:getCurrent().location.endd.line + ((self.length > 0 and self.buffer[self.length - 1] ~= '\n' and 0) or 1);
    ...
    blockFollow = function(self, token)
        return  token.token_type == TokenType.Eof or token.token_type == TokenType.ReservedElse or token.token_type == TokenType.ReservedElseif or
                token.token_type == TokenType.ReservedEnd or token.token_type == TokenType.ReservedUntil;
    end,
    ...
    parseStat = function(self)
        ...
        if self.tokenizer:getCurrent().token_type == TokenType:getChar(',') or self.tokenizer:getCurrent().token_type == TokenType:getChar('=') then
        ...
        if identifier == "export" and self.tokenizer:getCurrent().token_type == TokenType.Name and AstName.new(self.tokenizer:getCurrent().value).value == "type" then
            self:nextToken();
        ...
        return self:reportStatError(expression.location, expression, nil, "Incomplete statement: expected assignment or function call");
    end,
    ...
    parseIf = function(self)
        ...
        return AstStatIf.new(Location.new(start, endd), condition, then_body, else_body, then_location, else_location, has_end);
    end,
    ...
    parseContinue = function(self, start)
        if self.function_stack:getBack().loop_depth == 0 then
            return self:reportStatError(start, nil, AstStatContinue.new(start), "continue statement must be inside a loop");
        end;
    ...
    parseFor = function(self)
        ...
        if self.tokenizer:getCurrent().token_type == TokenType:getChar('=') then
            ...
            return AstStatFor.new(Location.new(start, endd), var, from, to, step, body, has_do, match_do.location, has_end);
        else
            ...
            return AstStatForIn.new(
                Location.new(start, endd), vars, values, body, has_in, in_location, has_do, match_do.location, has_end
            );
        end;
    end,
    ...
    parseFunctionName = function(self, start)
        ...
        while self.tokenizer:getCurrent().token_type == TokenType:getChar('.') do
            ...
            expression = AstExpressionIndexName.new(Location.new(start, name.location), expression, name.name, name.location, op_position, '.');
    ...
    parseFunctionStat = function(self)
        ...
        self.match_recovery_stop_on_token[TokenType.ReservedEnd.value] = self.match_recovery_stop_on_token[TokenType.ReservedEnd.value] + 1;
    ...
    parseLocal = function(self)
        ...
        if self.tokenizer:getCurrent().token_type == TokenType.ReservedFunction then
            ...
            self.match_recovery_stop_on_token[TokenType:getChar('=').value] = self.match_recovery_stop_on_token[TokenType:getChar('=').value] + 1;
    ...
    parseReturn = function(self)
        ...
        if not self:blockFollow(self.tokenizer:getCurrent()) and self.tokenizer:getCurrent().token_type ~= TokenType:getChar(';') then
    ...
    parseTypeAlias = function(self, start, is_exported)
        ...
        return AstStatTypeAlias.new(Location.new(start, ttype.location), name.name, name.location, generics, generic_packs, ttype, is_exported);
    end,
    ...
    parseDeclaredClassMethod = function(self)
        ...
        if args.size == 0 or args[1].name.name ~= "self" or args[1].annotation ~= nil then
            return AstDeclaredClassProp.new(function_name.name, self:reportTypeError(Location.new(start, endd), nil, "'self' must be present as the unannotated first parameter"), true);
        end;
        ...
        for i = 2, args.size do
            ...
            if args[i].annotation then
                ...
                vars:pushBack(self:reportTypeError(Location.new(start, endd), nil, "declaration parameters aside from 'self' must be annotated"));
            end;
        end;
        ...
        local function_type = AstTypeFunction.new(Location.new(start, endd), generics, generic_packs, AstTypeList.new(vars, var_arg_annotation), var_names, return_types);
    ...
    parseDeclaration = function(self, start)
        if true then error("parseDeclaration should not be used until it can be verified with cpp LuaU's"); end
            ...
            local global_name = self:parseName("global function name");
                ...
                for i = 1, args.size do
                    if not args[i].annotation then
                        return self:reportStatError(Location.new(start, endd), nil, nil, "All declaration parameters must be annotated");
                    end;
                ...
                return AstStatDeclareFunction.new(
                    Location.new(start, endd), global_name.name, generics, generic_packs,
                    AstTypeList.new(vars, var_arg_annotation), var_names, return_types
                );
        elseif AstName.new(self.tokenizer:getCurrent().name).value == "class" then
            ...
            while self.tokenizer:getCurrent().token_type ~= TokenType.ReservedEnd do
                ...
                elseif self.tokenizer:getCurrent().token_type == TokenType:getChar('[') then
                    if not FFlag.LuauParseDeclareClassIndexer or self.tokenizer:lookAhead().token_type == TokenType.RawString or self.tokenizer:lookAhead().token_type == TokenType.QuotedString then
                    ...
                    elseif FFlag.LuauParseDeclareClassIndexer then
                        if indexer then
                            ...
                            self:report(bad_indexer.location, "Cannot have more than one class indexer");
                        else
            ...
            return AstStatDeclareClass.new(Location.new(class_start, class_end), class_name.name, super_name, props, indexer);
        else
            ...
            if global_name then
                ...
                return self:reportStatError(start, nil, nil, "declare must be followed by an identifier, 'function', or 'class'");
            end;
        end;
    end,
    ...
    parseAssignment = function(self, initial)
        if not isExprLValue(initial) then
            initial = self:reportExpressionError(initial.location, initial, "Assigned expression must be a variable or a field");
        end;
        ...
        while self.tokenizer:getCurrent().token_type == TokenType:getChar(',') do
            ...
            if not isExprLValue(expression) then
                expression = self:reportExpressionError(expression.location, expression, "Assigned expression must be a variable or a field");
            end;
    ...
    parseFunctionBody = function(self, has_self, match_function, debug_name, local_name)
        ...
        if match_parenthesis.token_type == TokenType:getChar('(') and self.tokenizer:getCurrent().token_type == TokenType:getChar(')') then
        ...
        return AstExpressionFunction.new(Location.new(start, endd), generics, generic_packs, sself, vars, var_arg,
                                        var_arg_location, body, self.function_stack.size, debug_name, type_list,
                                        var_arg_annotation, has_end, arg_location
        ), func_local;
    end,
    ...
    parseTypeList = function(self, result, result_names)
        while true do
            ...
            if self.tokenizer:getCurrent().token_type == TokenType.Name and self.tokenizer:lookAhead().token_type == TokenType:getChar(':') then
                -- Fill in previous argument names with empty slots
                while result_names.size < result.size do
                    result_names:pushBack({});
                end;

                result_names:pushBack(AstArgumentName.new(AstExpressionIndexName.new(AstName.new(self.tokenizer:getCurrent().location))));
                self:nextToken();
    ...
    parseOptionalReturnType = function(self)
        if self.tokenizer:getCurrent().token_type == TokenType:getChar(':') or self.tokenizer:getCurrent().token_type == TokenType.SkinnyArrow then
            if self.tokenizer:getCurrent().token_type == TokenType.SkinnyArrow then
                self:report(self.tokenizer:getCurrent().location, "Function return type annotations are written after ':' instead of '->'");
            end;
            ...
            local location, result = self:parseReturnType();
            ...
            if self.tokenizer:getCurrent().token_type == TokenType:getChar(',') then
                self:report(self.tokenizer:getCurrent().location, "Expected a statement, got ','; did you forget to wrap the list of return types in parentheses?");
    ...
    parseReturnType = function(self)
        ...
        self.match_recovery_stop_on_token[TokenType.SkinnyArrow.value] = self.match_recovery_stop_on_token[TokenType.SkinnyArrow.value] + 1;
    ...
    parseTableType = function(self)
        ...
        while self.tokenizer:getCurrent().token_type ~= TokenType:getChar('}') do
            if self.tokenizer:getCurrent().token_type == TokenType:getChar('[') and (self.tokenizer:lookAhead().token_type == TokenType.RawString or self.tokenizer:lookAhead().token_type == TokenType.QuotedString) then
                local begin = self.tokenizer:getCurrent();
                self:nextToken(); -- [
                local chars = self:parseCharArray();
            ...
            elseif props:isEmpty() and not indexer and not (self.tokenizer:getCurrent().token_type == TokenType.Name and self.tokenizer:lookAhead().token_type == TokenType:getChar(':')) then
                local ttype = self:parseType();

                -- array-like table type: {T} desugars into {[number]: T}
                local index = AstTypeReference.new(ttype.location, nil, self.name_number, nil, ttype.location):toAstType();
    ...
    parseFunctionType = function(self, allow_pack)
        ...
        local return_type_introducer = self.tokenizer:getCurrent().token_type == TokenType.SkinnyArrow or self.tokenizer:getCurrent().token_type == TokenType:getChar(':');
    ...
    parseFunctionTypeTail = function(self, begin, generics, generic_packs, params, param_names, var_arg_annotation)
        ...
        if self.tokenizer:getCurrent().token_type == TokenType:getChar(':') then
            self:report(self.tokenizer:getCurrent().location, "Return types in function type annotations are written after '->' instead of ':'");
        ...
        elseif self.tokenizer:getCurrent().token_type ~= TokenType.SkinnyArrow and generics.size == 0 and generic_packs.size == 0 and params.size == 0 then
            self:report(Location.new(begin.location, self.tokenizer:getPreviousLocation()), "Expected '->' after '()' when parsing function type; did you mean 'nil'?");
        ...
        return AstTypeFunction.new(Location.new(begin.location, end_location), generics, generic_packs, param_types, param_names, return_type_list);
    end,
    ...
    parseTypeSuffix = function(self, ttype, begin)
        ...
        if is_union and is_intersection then
            return self:reportTypeError(Location.new(begin, parts:getBack().location), parts, "Mixing union and intersection types is not allowed; consider wrapping in parentheses.");
        end;
    ...
    parseSimpleType = function(self, allow_pack)
        ...
        elseif self.tokenizer:getCurrent().token_type == TokenType.RawString or self.tokenizer:getCurrent().token_type == TokenType.QuotedString then
        ...
        elseif self.tokenizer:getCurrent().token_type == TokenType.InterpolatedStringBegin or self.tokenizer:getCurrent().token_type == TokenType.InterpolatedStringSimple then
        ...
        elseif self.tokenizer:getCurrent().token_type == TokenType.Name then
            ...
            elseif self.tokenizer:getCurrent().token_type == TokenType.Dot3 then
                self:report(self.tokenizer:getCurrent().location, "Unexpected '...' after type name; a type pack is not allowed in this context");
                self:nextToken();
            ...
            return AstTypeReference.new(Location.new(start, endd), prefix, name.name, prefix_location, name.location, has_parameters, parameters);
        elseif self.tokenizer:getCurrent().token_type == TokenType:getChar('{') then
            return self:parseTableType();
        elseif self.tokenizer:getCurrent().token_type == TokenType:getChar('(') or self.tokenizer:getCurrent().token_type == TokenType:getChar('<') then
            return self:parseFunctionType(allow_pack);
        elseif self.tokenizer:getCurrent().token_type == TokenType.ReservedFunction then
            self:nextToken();

            return self:reportTypeError(start, nil, 
            ...
            return self:reportMissingTypeError(parse_error_location, ast_error_location, "Expected type, got %s", self.tokenizer:getCurrent():toString());
        end;
    end,

    parseVariadicArgumentTypePack = function(self)
        -- Generic: a...
        if self.tokenizer:getCurrent().token_type == TokenType.Name and self.tokenizer:lookAhead().token_type == TokenType.Dot3 then
    ...
    parseTypePack = function(self)
        ...
        -- Generic: a...
        elseif self.tokenizer:getCurrent().token_type == TokenType.Name and self.tokenizer:lookAhead().token_type == TokenType.Dot3 then
            local name = self:parseName("generic name");
            local endd = self.tokenizer:getCurrent().location;
    ...
    checkBinaryConfusables = function(self, binary_priority, limit)
        ...
        -- early out: need to check if this is a possible confusable quickly
        if current.token_type ~= TokenType:getChar('&') and current.token_type ~= TokenType:getChar('|') and current.token_type ~= TokenType:getChar('!') then
            return;
        end;
        ...
        if current.token_type == TokenType:getChar('&') and after.token_type == TokenType:getChar('&') and current.location.endd == after.location.begin and binary_priority[AstExpressionBinary.enum.Add.value].left > limit then
            self:nextToken();
            self:report(Location.new(start, after.location), "Unexpected '&&', did you mean 'and'?");
            return AstExpressionBinary.enum.And;
        elseif current.token_type == TokenType:getChar('|') and after.token_type == TokenType:getChar('|') and current.location.endd == after.location.begin and binary_priority[AstExpressionBinary.enum.Or.value].left > limit then
            self:nextToken();
            self:report(Location.new(start, after.location), "Unexpected 'or' did you mean 'and'?");
            return AstExpressionBinary.enum.Or;
        elseif current.token_type == TokenType:getChar('!') and after.token_type == TokenType:getChar('=') and current.location.endd == after.location.begin and
        binary_priority[AstExpressionBinary.enum.CompareNe.value].left > limit then
            self:nextToken();
            self:report(Location.new(start, after.location), "Unexpected '!=' did you mean 'and'?");
            return AstExpressionBinary.enum.CompareNe;
        end;
    ...
    parseExpression = function(self, limit)
        ...
        if unary_operation then
            ...
            expression = AstExpressionUnary.new(Location.new(start, sub_expression.location), unary_operation, sub_expression);
        else
    ...
    parsePrefixExpression = function(self)
        if self.tokenizer:getCurrent().token_type == TokenType:getChar('(') then
            ...
            if self.tokenizer:getCurrent().token_type ~= TokenType:getChar(')') then
                local suggestion = (self.tokenizer:getCurrent().token_type == TokenType:getChar('=') and "; did you mean to use '{' when defining a table?") or nil;
    ...
    parsePrimaryExpression = function(self, as_statement)
        ...
        while true do
            if self.tokenizer:getCurrent().token_type == TokenType:getChar('.') then
                ...
                expression = AstExpressionIndexName.new(Location.new(start, index.location.endd), expression, index.name, index.location, position, '.');
            elseif self.tokenizer:getCurrent().token_type == TokenType:getChar('[') then
            ...
            elseif self.tokenizer:getCurrent().token_type == TokenType:getChar(':') then
                ...
                local func = AstExpressionIndexName.new(Location.new(start, index.location.endd), expression, index.name, index.location, position, ':');
            ...
            elseif self.tokenizer:getCurrent().token_type == TokenType:getChar('(') then
                ...
                if (not as_statement) and (expression.location.endd.line ~= self.tokenizer:getCurrent().location.begin.line) then
                    self:reportAmbiguousCallError();
                    break;
                end;

                expression = self:parseFunctionArgs(expression, false);
            elseif self.tokenizer:getCurrent().token_type == TokenType:getChar('{') or self.tokenizer:getCurrent().token_type == TokenType.RawString or self.tokenizer:getCurrent().token_type == TokenType.QuotedString then
                expression = self:parseFunctionArgs(expression, false);
            else
                break;
            end;
    ...
    parseSimpleExpression = function(self)
        ...
        elseif  self.tokenizer:getCurrent().token_type == TokenType.RawString or
                self.tokenizer:getCurrent().token_type == TokenType.QuotedString or
                self.tokenizer:getCurrent().token_type == TokenType.InterpolatedStringSimple then
    ...
    parseFunctionArgs = function(self, func, has_self)
        if self.tokenizer:getCurrent().token_type == TokenType:getChar('(') then
            ...
            return AstExpressionCall.new(Location.new(func.location, endd), func, args, has_self, Location.new(arg_start, arg_end));
        elseif self.tokenizer:getCurrent().token_type == TokenType:getChar('{') then
            local arg_start = self.tokenizer:getCurrent().location.endd;
            local expression = self:parseTableConstructor();
            local arg_end = self.tokenizer:getPreviousLocation().endd;

            return AstExpressionCall.new(Location.new(func.location, expression.location), func, expression, has_self, Location.new(arg_start, arg_end));
        elseif self.tokenizer:getCurrent().token_type == TokenType.RawString or self.tokenizer:getCurrent().token_type == TokenType.QuotedString then
            local arg_location = self.tokenizer:getCurrent().location;
            local expression = self:parseString();

            return AstExpressionCall.new(Location.new(func.location, expression.location), func, expression, has_self, arg_location);
        else
    ...
    reportFunctionArgsError = function(self, func, has_self)
        if has_self and self.tokenizer:getCurrent().location.begin.line ~= func.location.endd.line then
            ...
            return self:reportExpressionError(Location.new(func.location.begin, self.tokenizer:getCurrent().location.begin), func, "Expected '(', '{' or <string> when parsing function call, got %s", self.tokenizer:getCurrent():toString());
        end;
    end,

    reportAmbiguousCallError = function(self)
        self:report(self.tokenizer:getCurrent().location, "Ambiguous syntax: this looks like an argument list for a function call, " ..
    ...
    parseTableConstructor = function(self)
        ...
        while self.tokenizer:getCurrent().token_type ~= TokenType:getChar('}') do
            ...
            elseif self.tokenizer:getCurrent().token_type == TokenType.Name and self.tokenizer:lookAhead().token_type == TokenType:getChar('=') then
            ...
            if self.tokenizer:getCurrent().token_type == TokenType:getChar(',') or self.tokenizer:getCurrent().token_type == TokenType:getChar(';') then
                self:nextToken();
            elseif (self.tokenizer:getCurrent().token_type == TokenType:getChar('[') or self.tokenizer:getCurrent().token_type == TokenType.Name) and self.tokenizer:getCurrent().location.begin.column == last_element_indent then
    ...
    parseIfElseExpression = function(self)
        ...
        return AstExpressionIfElse.new(Location.new(start, endd), condition, has_then, true_expression, has_else, false_expression);
    end,
    ...
    parseIndexName = function(self, context, previous)
        ...
        if  self.tokenizer:getCurrent().token_type >= TokenType.Reserved_BEGIN and
            self.tokenizer:getCurrent().token_type < TokenType.Reserved_END and
            self.tokenizer:getCurrent().location.begin.line == previous.line then
        ...
        if self.tokenizer:getCurrent().token_type == TokenType:getChar('<') then
            ...
            while true do
                ...
                if self.tokenizer:getCurrent().token_type == TokenType.Dot3 or seen_pack then
                    ...
                    if self.tokenizer:getCurrent().token_type ~= TokenType.Dot3 then
                        self:report(self.tokenizer:getCurrent().location, "Generic types come before generic type packs");
                    else
                        self:nextToken();
                    end;
                    ...
                    if with_default_values and self.tokenizer:getCurrent().token_type == TokenType:getChar('=') then
                        ...
                        if seen_default then
                            self:report(self.tokenizer:getCurrent().location, "Expected default type pack after type pack name");
                        end;
    ...
    parseCharArray = function(self)
        assert( self.tokenizer:getCurrent().token_type == TokenType.QuotedString or
                self.tokenizer:getCurrent().token_type == TokenType.RawString or
                self.tokenizer:getCurrent().token_type == TokenType.InterpolatedStringSimple, "parseCharArray saw an invalid current token type: " .. tostring(self.tokenizer:getCurrent().token_type)
        );

        local data = self.tokenizer:getCurrent().value;

        if self.tokenizer:getCurrent().token_type == TokenType.QuotedString or self.tokenizer:getCurrent().token_type == TokenType.InterpolatedStringSimple then
    ...
    parseInterpolatedString = function(self)
        ...
        repeat
            ...
            assert( current_token.token_type == TokenType.InterpolatedStringBegin or current_token.token_type == TokenType.InterpolatedStringMid or
                    current_token.token_type == TokenType.InterpolatedStringEnd or current_token.token_type == TokenType.InterpolatedStringSimple, "parseInterpolatedString saw an invalid current token type: " .. tostring(self.tokenizer:getCurrent().token_type)
            );
            ...
            if not self.tokenizer:fixupQuotedString(self.scratch_data) then
                self:nextToken();
                return self:reportExpressionError(Location.new(start_location, end_location), nil, "Interpolated string literal contains maliformed escape sequence");
            end;
            ...
            if current_token.token_type == TokenType.InterpolatedStringEnd or current_token.token_type == TokenType.InterpolatedStringSimple then
                break;
            end;
            ...
            if current_type == TokenType.InterpolatedStringMid or current_type == TokenType.InterpolatedStringEnd then
                ...
                self:nextToken();
                expressions:pushBack(self:reportExpressionError(end_location, nil, "Maliformed interpolated string, expected expression inside '{}'"));
            elseif current_type == TokenType.BrokenString then
                got_error_while_checking = true;
                self:nextToken();
                expressions:pushBack(self:reportExpressionError(end_location, nil, "Maliformed interpolated string, did you forget to add a '`'?"));
            else
            ...
            if current_type == TokenType.InterpolatedStringBegin or current_type == TokenType.InterpolatedStringMid or current_type == TokenType.InterpolatedStringEnd then
            ...
            elseif current_type == TokenType.BrokenString then
                self:nextToken();
                return self:reportExprError(end_location, nil, "Malformed interpolated string, did you forget to add a '}'?");
            else
                return self:reportExprError(end_location, nil, "Malformed interpolated string, got %s", self.tokenizer:getCurrent():toString());
            end;
        until false;

        local strings_array = strings;
        local expressions_array = expressions;
        return AstExpressionInterpolatedString.new(Location.new(start_location, end_location), strings_array, expressions_array);
    end,
    ...
    pushLocal = function(self, binding)
        ...
        llocal = AstLocal.new(
            name.name, name.location, --[[shadow =]] llocal, self.function_stack.size,
            self.function_stack:getBack().loop_depth, binding.annotation
        );
    ...
    expectAndConsumeFail = function(self, ttype, context)
        ...
        if context then
            self:report(self.tokenizer:getCurrent().location, "Expected %s when parsing %s, got %s", type_string, context, current_token_string);
        else
    ...
    expectMatchAndConsumeRecover = function(self, ttype, begin, search_for_missing)
        if search_for_missing then
            ...
            while current_line == self.tokenizer:getCurrent().location.begin.line and token_type ~= ttype and self.match_recovery_stop_on_token[ttype.value] == 0 do
                self:nextToken();
    ...
    expectMatchAndConsumeFail = function(self, ttype, begin, extra)
        ...
        if self.tokenizer:getCurrent().location.begin.line == begin.position.line then
            self:report(self.tokenizer:getCurrent().location, "Expected %s (to close %s at column %d), got %s%s", type_string, match_string,
                begin.position.column + 1, self.tokenizer:getCurrent():toString(), extra or "");
        else
            self:report(self.tokenizer:getCurrent().location, "Expected %s (to close %s at line %d), got %s%s", type_string, match_string,
                begin.position.line + 1, self.tokenizer:getCurrent():toString(), extra or "");
        end;
    end,
    ...
    expectMatchEndAndConsume = function(self, ttype, begin)
        if self.tokenizer:getCurrent().token_type ~= ttype then
            ...
            if (self.tokenizer:getCurrent().location.begin.line ~= begin.position.line and self.tokenizer:getCurrent().location.begin.column ~= begin.position.column and
                self.end_mismatch_suspect.position.line < begin.position.line) then -- Only replace the previous suspect with more recent suspects
    ...
    expectMatchEndAndConsumeFail = function(self, ttype, begin)
        if self.end_mismatch_suspect.ttype ~= TokenType.Eof and self.end_mismatch_suspect.position.line > begin.position.line then
            local match_string = Token.new(Location.new(Position.new(0, 0), 0), self.end_mismatch_suspect.ttype):toString();
            local suggestion = ("; did you forget to close %s at line %d?"):format(match_string, self.end_mismatch_suspect.position.line + 1);
    ...
    incrementRecursionCounter = function(self, context)
        ...
        if self.recursion_counter > FInt.LuauRecursionLimit then
            ParseError.raise(self.tokenizer:getCurrent().location, "Exceeded allowed recursion depth; simplify your %s to make the code compile", context);
        end;
    end,
    ...
    report = function(self, location, format_string, ...)
        ...
        if FInt.LuauParseErrorLimit == 1 then
            ParseError.new(location, message):error();
        end;

        self.parse_errors:pushBack(ParseError.new(location, message));

        if self.parse_errors.size >= FInt.LuauParseErrorLimit then
            ParseError.raise(location, "Reached error limit (%d)", FInt.LuauParseErrorLimit);
        end;
    end,

    reportNameError = function(self, context)
        if context then
            self:report(self.tokenizer:getCurrent().location, "Expected identifier when parsing %s, got %s", context, self.tokenizer:getCurrent():toString());
        else
            self:report(self.tokenizer:getCurrent().location, "Expected identifier, got %s", self.tokenizer:getCurrent():toString());
        end;
    end,
    ...
    nextToken = function(self)
        ...
        while token_type == TokenType.BrokenComment or token_type == TokenType.Comment or token_type == TokenType.BlockComment do
