...
local function enum(items)
    ...
    local enum = setmetatable({
        ...
        fromValue = function(_, value)
            return value_map[value] or error("failed to find item with value '" .. tostring(value) .. "' in enum '" .. (enum.name or "[UNNAMED ENUM]") .. "'", 2);
        end
    }, {
...
local token_tostring_functions = {};do
...
local Token = class(function(self, location, token_type, value)
    if not isInstance(token_type, TokenType) then
        return error("Expected TokenType for second arg to Token.new", 3);
    end;
...
local Location; Location = class(function(self, begin, endd)
    ...
    checkOverlaps = function(self, other)
        return (self.begin <= other.begin and self.endd >= other.begin) or (self.begin <= other.endd and self.endd >= other.endd) or (self.begin >= other.begin and self.endd <= other.endd);
    end,
...
local TempVector; TempVector = class(function(self, storage)
...
local AstNameTable = class(function(self)
    ...
    assert(#kReserved == TokenType.Reserved_END - TokenType.Reserved_BEGIN, "kReserved table does not contain the same number of elements as there are reserved token types");
...
local AstLocal = simpleclass{name = "AstLocal", "name", "location", "shadow", "function_depth", "loop_depth", "annotation"};
...
local AstExpressionFunction = class(function(self, location, generics, generic_packs, has_self, args, has_var_arg, var_arg_location, body, function_depth, debug_name, return_annotation, var_arg_annotation, has_end, arg_location)
...
local unary_tostring_functions = {};do
...
local AstExpressionIfElse = class(function(self, location, condition, has_then, true_expression, has_else, false_expression)
...
local AstStatForIn = class(function(self, location, vars, values, body, has_in, in_location, has_do, do_location, has_end)
...
local AstStatDeclareFunction = class(function(self, location, name, generics, generic_packs, params, param_names, return_types)
...
local AstTypeReference = class(function(self, location, prefix, name, prefix_location, name_location, has_parameter_list, parameters)
...
local function getLocation(type_list)
    ...
    if type_list.types.size > 0 then
        result = Location.new(type_list.types.elements[1].location, type_list.types.elements[type_list.types.size].location);
    else
        result = Location.new();
    end;
    ...
    return {
        ...
        set = function(self, key, value)
            if type(value) ~= t then
                return error("class '" .. name .. "' only allows for the type '" .. t .. "', not '" .. type(value) .. "'", 2);
            end;
            self[key] = value;
        end;
    };
