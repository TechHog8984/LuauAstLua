...
local isInstance, class, newBuffer = shared.isInstance, shared.class, shared.newBuffer;
...
local Tokenizer = class(function(self, content, names)
    ...
    next = function(self, skip_comments, update_previous_loation)
        ...
        until not (skip_comments and (self.token.token_type == Token.Comment or self.token.token_type == Token.BlockComment));
    ...
    readCommentBody = function(self)
        ...
        return Token.new(Location.new(start, self:getPosition()), TokenType.Comment, self.buffer:fromTo(start_offset, self.offset));
    end,
        ...
        while self:peek() ~= 0 do
            if self:peek() == ']' then
                if self:skipLongSeparator() == separator then
                    ...
                    return Token.new(Location.new(start, self:getPosition()), ok, self.buffer:fromTo(start_offset, end_offset));
                end;
            else
                self:consume();
            end;
    ...
    readQuotedString = function(self)
        ...
        return Token.new(Location.new(start, self:getPosition()), TokenType.QuotedString, self.buffer:fromTo(start_offset, self.offset - 2));
    end,
    ...
    readInterpolatedStringBegin = function(self)
        ...
        return self:readInterpolatedStringSection(start, TokenType.InterpolatedStringBegin, TokenType.InterpolatedStringSimple);
    end,
    ...
    readInterpolatedStringSection = function(self, start, format_type, end_type)
        ...
        while self:peek() ~= '`' do
            ...
            elseif ch == '{' then
                ...
                if (self:peek(1) == '{') then
                    local broken_double_brace = Token.new(Location.new(start, self:getPosition()), TokenType.BrokenInterpolatedDoubleBrace, self.buffer:fromTo(start_offset, self.offset));
                    self:consume();
                    self:consume();
                ...
                self:consume();
                return Token.new(Location.new(start, self:getPosition()), format_type, self.buffer:fromTo(start_offset, self.offset - 1));
            else
                self:consume();
            end;
        end;
        ...
        return Token.new(Location.new(start, self:getPosition()), end_type, self.buffer:fromTo(start_offset, self.offset - 1));
    end,
    ...
    readNumber = function(self, start, start_offset)
        ...
        return Token.new(Location.new(start, self:getPosition()), TokenType.Number, self.buffer:fromTo(start_offset, self.offset - 1));
    end,
    ...
    readNext = function(self)
        ...
        elseif ch == '}' then
            ...
            return self:readInterpolatedStringSection(self:getPosition(), TokenType.InterpolatedStringMid, TokenType.InterpolatedStringEnd);
